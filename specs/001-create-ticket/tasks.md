...
**현재 상태**: 계획 수립 시점 기준 이 엔드포인트는 이미 대부분 구현되어 있다(research.md 참고). 아래
태스크 중 이미 완료된 항목은 `[x]`로 표시했다. 실제로 남은 작업은 T012, T014(누락된 회귀 테스트
2건)와 T016, T017(검증)뿐이다.
...
- [ ] T012 [US3] TC-API-001-4 테스트 작성: title이 공백 문자로만 구성된 경우 400 + "제목을
  입력해주세요" 검증 — `__tests__/api/tickets.test.ts` (의존: T010)
- [x] T013 [US3] TC-API-001-5 테스트 작성: title이 200자를 초과하면 400 + "제목은 200자 이내로
  입력해주세요" 검증 — `__tests__/api/tickets.test.ts` (의존: T010)
- [ ] T014 [US3] TC-API-001-6 테스트 작성: description이 1000자를 초과하면 400 + "설명은 1000자
...
- [ ] T016 [P] `quickstart.md`의 curl 시나리오 3종을 로컬 서버에서 수동 검증
- [ ] T017 [P] `npx tsc --noEmit`, `npm run lint`, `npm test`가 모두 통과하는지 확인
...
### 현재 상태 기준 남은 작업 (Remaining Work)

1. T012 작성 — 제목 공백만 입력 회귀 테스트
2. T014 작성 — 설명 1000자 초과 회귀 테스트
3. T016 — quickstart.md 수동 검증
4. T017 — 타입체크/lint/전체 테스트 통과 확인

### MVP 범위

User Story 1(T006, T007)은 이미 완료되어 있어 MVP는 이미 배포 가능한 상태다. 남은 작업은 P3
스토리의 회귀 테스트 커버리지 보강과 최종 검증뿐이다.
...
- T012, T014를 제외한 모든 테스트는 이미 통과 중이다(`npm test -- __tests__/api/tickets.test.ts`
  기준 6/6 통과, 전체 8개 시나리오 중 2개 테스트 미작성)
- 커밋은 태스크 단위로 수행한다
